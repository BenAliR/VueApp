...
export const useInviteStore = defineStore("invite", () => {
  ...
  function setInviteError(error) {
    errors.value = { ...error };
  }
  function CreateInvite(credentials) {
    if (JwtService.getToken()) {
      ApiService.setHeader();
      return ApiService.post("admin/users/invites/add", credentials)
        .then(() => {
          setInviteError({});
        })
        .catch(({ response }) => {
  ...
  function CreateInviteTask(credentials) {
    if (JwtService.getToken()) {
      ApiService.setHeader();
      return ApiService.post("invite/task/add", credentials)
        .then(() => {
          setInviteError({});
        })
        .catch(({ response }) => {
  ...
  function UpdateTaskStatus(credentials, id) {
    if (JwtService.getToken()) {
      ...
      return ApiService.post(
        "admin/users/invites/task/update/status/" + id,
        credentials
      )
        .then(() => {
          setInviteError({});
  ...
  function UpdateTask(credentials, id) {
    if (JwtService.getToken()) {
      ApiService.setHeader();
      return ApiService.post(
        "admin/users/invites/task/update/" + id,
        credentials
      )
        .then(() => {
          setInviteError({});
  ...
  function DeleteTaskComment(id) {
    if (JwtService.getToken()) {
      ApiService.setHeader();
      return ApiService.delete("admin/users/invites/task/comment/delete/" + id)
        .then(() => {
          setInviteError({});
        })
        .catch(({ response }) => {
  ...
  function getInvites() {
    if (JwtService.getToken()) {
      ApiService.setHeader();
      ApiService.get("admin/invite/list")
        .then(({ data }) => {
          return data;
        })
        .catch(({ response }) => {
