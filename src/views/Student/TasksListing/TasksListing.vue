<template>
  ...
  <!--end::Row-->
  <div  v-if="tasks.length" class="card card-custom">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        <div
            ...
            data-kt-task-table-toolbar="base"
        >
          <!--begin::Export-->
          <button
              type="button"
              ...
              data-bs-target="#kt_student_tasks_export_modal"
          >
    ...
    <div class="card-body pt-0 dataTables_wrapper dt-bootstrap4">
      <Datatable
          ...
          checkbox-label="id"
      >
        <template v-slot:title="{ row: task }">
          ...
          <router-link
              ...
              :to="{
             name: 'task-overview',
          params: { id: task.slug ,slug:task.project.slug},
            }"
          >
...
<!--          >-->
        ...
        <template v-slot:priority="{ row: task }">
          <span
              :class="getStatusColorClass(task.priority)"
              class="badge fw-bold me-auto px-4 py-3"
          >{{ getPriority(task.priority) }}</span
          >
        </template>
        <template v-slot:type="{ row: task }">
          <span
              :class="getStatusTypeColorClass(task.type)"
              class="badge fw-bold me-auto px-4 py-3"
          >{{ capitalizeFirstLetter(task.type) }}</span
          >
        </template>

        <template v-slot:status="{ row: task }">
          <span
              :class="TaskColorStatus(task.status)"
              class="badge fw-bold me-auto px-4 py-3"
        ...
        <template v-slot:creator="{ row: task }">
          <router-link
              class="text-gray-600 text-hover-primary mb-1"
              :to="{
              name: 'students-overview',
              params: { id: task.creator.id },
            }"
          >
            {{ task.creator.fullname }}</router-link
          >
        </template>
        <template v-slot:created_at="{ row: task }">
  ...
  <div  v-if="tasks.length"
        class="modal fade"
      id="kt_student_tasks_export_modal"
      tabindex="-1"
      aria-hidden="true"
  >
    <!--begin::Modal dialog-->
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<ITask>>(tasks.value);
    const initTasks = ref<Array<ITask>>([]);
    const filters = ref({
      title: "",
      status: "",
    ...
    const filteredTableData = computed(() => {
      const { title, status /* Add more filters here */ } =
          filters.value;

      return tableData.value.filter((task) => {
        const titleMatch = task.creator.fullname
            .toLowerCase()
    ...
    const deleteFewTasks = () => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          // store.StudentMultipeTask(formData);
    ...
    const searchItems = () => {
      tableData.value.splice(0, tableData.value.length, ...initTasks.value);
      if (search.value !== "") {
        let results: Array<ITask> = [];
    ...
    function filterUsersByDateRange(usersDetailsArray, startDate, endDate) {
      filteredUsers.value = usersDetailsArray.value.filter((taskDetails) => {
        // Convert created_at dates to JavaScript Date objects
        const createdAtDate = new Date(taskDetails.created_at);
    ...
    function generatePDF() {
      ...
      // Add page title
      doc.text("List of Tasks", 105, 20, { align: "center" });
      ...
      const tableData2 = filteredUsers.value.map((taskDetails) => [
        taskDetails["title"],
        capitalizeFirstLetter(TaskStatus(taskDetails["priority"])),
        taskDetails["creator"]["fullname"],
        taskDetails["type"],
    ...
    function TaskColorStatus(status) {
      ...
      if (status === "1") {
        ...
        return "badge-light-danger"; // Default color if status is null or unknown
      }
    };
    function getStatusTypeColorClass(type) {

      if (type === "Concept de design") {
        return "badge-light-warning";
      } else if (type === "Présentation du projet") {
        return "badge-light-primary";
      } else if (type === "Logiques fonctionnelles") {
        return "badge-light-success";
      }
    ...
    const endIndex = computed(() => {
      ...
      return end > filteredTasks.value.length
    ...
    const filteredTasks = computed(() => {
      if (statusFilter.value) {
        return tasks.value.filter(
            (task) => task["priority"] === statusFilter.value
        );
      }
      return tasks.value;
    });
    ...
    const taskStatuses = computed(() => {
      ...
      tasks.value.forEach((task) => {
        if (statusCounts[task["priority"]]) {
          statusCounts[task["priority"]]++;
        } else {
          statusCounts[task["priority"]] = 1;
        }
      });
      ...
      return Object.keys(statusCounts).map((status) => {
        ...
        return {
          title: count + " Tàches",
          description: getPriority(status),
          color,
          progress: percentage,
        };
      });
