 <template>
   ...
   <div class="row g-6 mb-6 g-xl-9 mb-xl-9">
     <!--begin::Followers-->
     <!--begin::Col-->
     <kt-empty v-if="!filteredConnections.length">
     </kt-empty>
     <div v-else
       ...
       <div class="card">
         ...
         <div class="card-body d-flex align-items-center pt-3 pb-0">
           <div class="d-flex flex-column flex-grow-1 py-2 py-lg-13 me-2">
             <a href="#" class="fw-bold text-dark fs-5 mb-2 text-hover-primary">{{ connection.fullname }}</a>
           </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      getProjectMemebers().finally(() => {
        // Disable loading indicator
        document.body.classList.remove("page-loading");
        document.body.removeAttribute("data-kt-app-page-loading");
      });
    });
    const getProjectMemebers = async () => {
      if (JwtService.getToken()) {
        try {
          ApiService.setHeader();
          const response = await ApiService.get(
              "admin/users/projects/members/" + route.params.id
          );
