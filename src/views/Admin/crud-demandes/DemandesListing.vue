<template>
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        <div
          ...
          data-kt-demande-table-toolbar="base"
        >
          <!--begin::Export-->
          <button
            type="button"
            ...
            data-bs-target="#kt_demandes_export_modal"
          >
          ...
          <button
            ...
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_add_demande"
          >
        ...
        <div
          ...
          data-kt-demande-table-toolbar="selected"
        >
        ...
        >
          <div class="fw-bold me-5">
            <span
              class="me-2"
              data-kt-demande-table-select="selected_count"
            ></span
            >Selected
          </div>
          <button
            type="button"
            class="btn btn-danger"
            data-kt-demande-table-select="delete_selected"
          >
            Delete Selected
          </button>
        </div>
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        checkbox-label="id"
      >
        <template v-slot:fullname="{ row: demande }">
          {{ demande.nom }} {{ demande.prenom }}
        </template>
        <template v-slot:email="{ row: demande }">
          <router-link
          ...
          >
            {{ demande.email }}</router-link
          >
        </template>
        <template v-slot:etablisement="{ row: demande }">
          {{ demande.etablisement }}
        </template>
        <template v-slot:demande_status="{ row: demande }">
          <span
        ...
        <template v-slot:created_at="{ row: demande }">
          {{ formatDate(demande.created_at) }}
        </template>

        <template v-slot:actions="{ row: demande }">
          <!--begin::Toolbar-->
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      ...
      <div class="modal-content">
        ...
        <div class="modal-header">
          <!--begin::Modal title-->
          <h2 class="fw-bold">Exporter le fichier Demandes</h2>
          <!--end::Modal title-->

          <!--begin::Close-->
          <div
            id="kt_demandes_export_close"
  ...
  <AddDemandeModal @after-hide="loadData"></AddDemandeModal>
</template>
...
import AddDemandeModal from "@/views/Admin/crud-demandes/create-demande-modal/AddDemandeModal.vue";
...
export default defineComponent({
  ...
  setup() {
    const tableHeader = ref([
      ...
      {
        columnName: "Etablisement",
        columnLabel: "etablisement",
        sortEnabled: true,
        columnWidth: 175,
      },
      {
        columnName: "Statut de demande",
        columnLabel: "demande_status",
        sortEnabled: true,
        columnWidth: 175,
      },
      {
    ...
    const tableData = ref<Array<IDemande>>(demandes.value);
    const initDemandes = ref<Array<IDemande>>([]);
    const store = useStudentStore();
    const getDemandes = async () => {
      if (JwtService.getToken()) {
        try {
          ApiService.setHeader();
          const response = await ApiService.get("admin/demandes/list");
    ...
    onMounted(async () => {
      ...
      tableData.value = [...demandes.value];
      initDemandes.value = [...demandes.value];
    ...
    const deleteFewDemandes = () => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          store.DeleteMultipeDemande(formData);
    ...
    const searchItems = () => {
      tableData.value.splice(0, tableData.value.length, ...initDemandes.value);
      if (search.value !== "") {
        let results: Array<IDemande> = [];
    ...
    function generatePDF() {
      ...
      // Add page title
      doc.text("List of Demandes", 105, 20, { align: "center" });

      // Set font styles for the table content
      doc.setFont("helvetica", "normal");
      doc.setFontSize(12);

      const tableHeaders = ["Nom", "Prenom", "Email", "telephone", "Statut"];
      const tableData2 = filteredUsers.value.map((userDetails) => [
        userDetails["nom"],
        userDetails["prenom"],
        userDetails["email"],
        userDetails["telephone"],
        userDetails["demande_status"],
      ]);
    ...
    function deleteItemDemande(id) {
      //Send  request
      store.DeleteDemande(id);
      const error = Object.values(store.errors);
