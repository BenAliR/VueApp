  ...
  <div class="card">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <button
            ...
            data-bs-target="#kt_students_export_modal"
          >
        ...
        <div
          ...
          data-kt-student-table-toolbar="selected"
        >
        ...
        >
          <div class="fw-bold me-5">
            <span
              class="me-2"
              data-kt-student-table-select="selected_count"
            ></span
            >Selected
          </div>
          <button
            type="button"
            class="btn btn-danger"
            data-kt-student-table-select="delete_selected"
          >
            Delete Selected
          </button>
        </div>
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        checkbox-label="id"
      >
        <template v-slot:fullname="{ row: student }">
          {{ student.fullname }}
        </template>
        <template v-slot:email="{ row: student }">
          ...
          >
            {{ student.email }}</router-link
          >
        </template>
        <template v-slot:last_login_at="{ row: student }">
          {{ student.last_login_at }}
        </template>
        <template v-slot:banned="{ row: student }">
          <span
            :class="{
              'badge badge-light-success': student.banned === '0',
              'badge badge-light-danger': student.banned === '1',
            }"
          >
            {{ student.banned === "0" ? "Active" : "Inactive" }}
          </span>
        </template>

        <template v-slot:created_at="{ row: student }">
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<IStudent>>(students.value);
    const initStudents = ref<Array<IStudent>>([]);
    const store = useStudentStore();
    const getStudents = async () => {
      if (JwtService.getToken()) {
        try {
          ApiService.setHeader();
          const response = await ApiService.get("admin/users/students/list");
    ...
    onMounted(async () => {
      ...
      tableData.value = [...students.value];
      initStudents.value = [...students.value];
    ...
    const deleteFewStudents = () => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          store.DeleteMultipeStudent(formData);
    ...
    function generatePDF() {
      ...
      // Add page title
      doc.text("List of Students", 105, 20, { align: "center" });
